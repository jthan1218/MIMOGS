#!/usr/bin/env python3
"""T1-cluster -- Nearest-neighbour vs. MIMO-GS on the entire-cluster DeepMIMO split.

Zero-argument runnable::

    python eval_t1_cluster.py

A two-row cut of the T1 table (``eval_t1.py``) on
``dataset/asu_campus_16by64_lt_entire_cluster``:
...
2. ``MIMO-GS``          -- ``outputs/20260831_103746/model.pth``, rendered
...
Outputs land in ``analysis/eval_t1_cluster/``.  Nothing in the repository is
modified.
...
# ---------------------------------------------------------------------------
DATASET_DIR = "dataset/asu_campus_16by64_lt_entire_cluster"
MIMOGS_RUN = "outputs/20260831_103746"
BATCH_SIZE = 32

OUTPUT_DIR = os.path.join(REPO_ROOT, "analysis", "eval_t1_cluster")
...
def assert_finite_nonnegative(maps: torch.Tensor, label: str) -> None:
    ...
    if not bool(torch.isfinite(maps).all()):
        raise AssertionError(f"[eval_t1_cluster] {label}: output contains non-finite values.")
    minimum = float(maps.min())
    if minimum < 0.0:
        raise AssertionError(
            f"[eval_t1_cluster] {label}: output has negative entries (min = {minimum:.6g})."
        )
...
class LoadedMIMOGS:
    ...
    def __init__(self, run_dir: str, dataset_dir: str, device: torch.device) -> None:
        ...
        if not os.path.isfile(checkpoint_path):
            raise SystemExit(
                f"[eval_t1_cluster] No '{ER.CHECKPOINT_NAME}' in '{run_dir}'."
            )
...
def main() -> int:
    ...
    print("=" * 84)
    print("[eval_t1_cluster] Nearest neighbour vs. MIMO-GS -- entire-cluster DeepMIMO split")
    print("=" * 84)

    for path in (dataset_dir, os.path.join(dataset_dir, "train.mat"),
                 os.path.join(dataset_dir, "test.mat"), run_dir):
        if not os.path.exists(path):
            raise SystemExit(f"[eval_t1_cluster] Required input is missing: {path}")
    ...
    print(f"[eval_t1_cluster] device            : {device}"
          + (f" ({torch.cuda.get_device_name(0)})" if device.type == "cuda" else ""))
    print(f"[eval_t1_cluster] dataset           : {relative(dataset_dir)}")
    print(f"[eval_t1_cluster] run               : {relative(run_dir)}")
    print(f"[eval_t1_cluster] test locations    : {n_test} (scored {n_scored}, "
          f"skipped zero-power {ground_truth.num_skipped_zero_power})")
    print(f"[eval_t1_cluster] beam grid         : "
          f"{ground_truth.beam_rows} x {ground_truth.beam_cols}")
    print(f"[eval_t1_cluster] pos_scale (test)  : {ground_truth.scale_factor:.6f}")
    print("")
    ...
    if tuple(nn_maps.shape[1:]) != (ground_truth.beam_rows, ground_truth.beam_cols):
        raise SystemExit(
            f"[eval_t1_cluster] Beam-grid mismatch: train.mat "
    ...
    print(f"[eval_t1_cluster] train locations   : {num_train} (full train.mat)")
    print(f"[eval_t1_cluster] {ROW_NN:<17}: "
    ...
    print(f"[eval_t1_cluster] {ROW_MIMOGS:<17}: "
    ...
    print("=" * 84)
    print("[eval_t1_cluster] TABLE  (shape NMSE, per-location, dB)")
    print("=" * 84)
    ...
    print("-" * 84)
    print("[eval_t1_cluster] NEAREST-TRAIN DISTANCE (scored test locations)")
    ...
    print("")
    print("[eval_t1_cluster] MIMO-GS vs. NEAREST NEIGHBOUR")
    ...
    print(f"[eval_t1_cluster] wrote {relative(summary_path)}")
    print(f"[eval_t1_cluster] wrote {relative(per_user_path)}")
    return 0
