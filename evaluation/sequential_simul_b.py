"""
sequential_simul_b.py

Replay / evaluation of the 11 data-ratio checkpoints produced by
sequential_b.py (ratio_000.pth ... ratio_100.pth).
...
magnitude maps from every data-ratio checkpoint (visualising how the model
improves as more training data is used), and it evaluates rendering-fidelity /
beam-selection metrics versus data ratio, saving plots, CSV and a JSON summary.
...
SCRIPT_NAME = "sequential_simul_b"
LOG_TAG = "[SequentialSimulB]"
EPS = 1e-8
...
def percent_from_path(path):
    """Extract the integer data-ratio percent from a ratio_XXX.pth filename."""
    name = os.path.basename(path)
    m = re.search(r"ratio_(\d+)\.pth$", name)
    if m is None:
        return None
...
def find_checkpoints(checkpoint_dir):
    """Return a list of (percent, path) sorted by percent for ratio_*.pth."""
    pattern = os.path.join(checkpoint_dir, "ratio_*.pth")
...
def save_sequence_grid(path, gt_vis, percent_vis_list, sample_idx, metric_by_pct):
    ...
    # Cells 1..11: predicted maps in ascending data-ratio order.
    im = None
    ...
    fig.suptitle(f"Test sample {sample_idx:05d}: data-ratio sequence")
    if im is not None:
...
def save_ratio_plot(path, percents, values, ylabel, title):
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(percents, values, marker="o", linestyle="-", color="tab:blue")
    ax.set_xlabel("data_ratio_percent")
...
def run(args):
    ...
    log(f"Output path: {output_path}", quiet)
    ...
    if not checkpoints:
        raise FileNotFoundError(
            f"No 'ratio_*.pth' checkpoints found in: {checkpoint_dir}"
        )
    ...
    if missing:
        log(f"WARNING: missing expected data-ratio percents {missing}; "
    ...
    for pct, ckpt_path in checkpoints:
        log(f"Evaluating ratio {pct:03d}% ...", quiet)
        ...
        with torch.no_grad():
            ...
            for idx in render_indices:
                ...
                sample_dir = os.path.join(render_root, f"sample_{idx:05d}")
                save_map_png(
                    os.path.join(sample_dir, f"ratio_{pct:03d}.png"),
                    pred_mag,
        ...
        log(f"ratio {pct:03d}%: shape_mse={mean_shape:.6f}, "
    ...
    with open(metrics_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "data_ratio_percent", "checkpoint_path", "num_eval_samples",
            "mean_shape_mse", "mean_raw_mse", "mean_raw_nmse",
            "mean_raw_nmse_db", "top1_accuracy",
        ])
        for r in metrics_rows:
            writer.writerow([
                r["data_ratio_percent"], r["checkpoint_path"], r["num_eval_samples"],
    ...
    with open(per_sample_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "data_ratio_percent", "sample_idx", "shape_mse", "raw_mse",
            "raw_nmse", "raw_nmse_db", "top1_correct",
        ])
        writer.writerows(per_sample_rows)

    # ----------------------------------------------------------------
    # Plots (x-axis = data_ratio_percent)
    # ----------------------------------------------------------------
    percents = [r["data_ratio_percent"] for r in metrics_rows]
    ...
    save_ratio_plot(
        os.path.join(output_path, "ratio_vs_shape_mse.png"),
        percents, shape_vals,
        "mean_shape_mse", "Shape MSE vs data ratio",
    )
    save_ratio_plot(
        os.path.join(output_path, "ratio_vs_nmse_db.png"),
        percents, nmse_db_vals,
        "mean_raw_nmse_db", "Raw NMSE (dB) vs data ratio",
    )
    save_ratio_plot(
        os.path.join(output_path, "ratio_vs_top1_accuracy.png"),
        percents, top1_vals,
        "top1_accuracy", "Top-1 accuracy vs data ratio",
    )
    ...
    def best_by(key, mode):
        ...
        return {"data_ratio_percent": r["data_ratio_percent"], key: r[key]}

    summary = {
        "checkpoint_dir": os.path.abspath(checkpoint_dir),
        "output_path": os.path.abspath(output_path),
        "evaluated_data_ratio_percents": percents,
        "missing_data_ratio_percents": missing,
        "test_set_size": test_size,
        "num_eval_samples": len(eval_indices),
        "selected_render_indices": render_indices,
        "selected_eval_indices": eval_indices,
        "best_ratio_by_shape_mse": best_by("mean_shape_mse", "min"),
        "best_ratio_by_nmse_db": best_by("mean_raw_nmse_db", "min"),
        "best_ratio_by_top1_accuracy": best_by("top1_accuracy", "max"),
    }
...
if __name__ == "__main__":
    parser = ArgumentParser(description="MIMOGS sequential_b data-ratio checkpoint replay / evaluation")

    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="outputs/sequential_b_20260622_080909/checkpoints",
    )
