...
def training(
    ...
    gaussians = GaussianModel(target_gaussians=int(getattr(model_params, "target_gaussians", 25_000)),optimizer_type=opt_params.optimizer_type,device=str(device),init_range=1.0,tie_covariance=bool(int(getattr(model_params, "tie_covariance", 0))),gain_pe_frequencies=int(getattr(model_params, "gain_pe_frequencies", 6)))
